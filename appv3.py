...
# Diccionario de logos de marcas conocidas (puedes expandir esta lista)
BRAND_LOGOS = {
...
def get_image_url_enhanced(item):
    """
    Función mejorada que busca imágenes usando prioridades:
    1. Imágenes de Ingram
    2. Logo de la marca
    3. Búsqueda externa con SerpApi
    ...
    # 2. Intentar obtener logo de la marca
    brand_name = item.get("vendorName", "")
    brand_logo = get_brand_logo(brand_name)
    if brand_logo:
        return brand_logo
    
    # 3. Buscar imagen externa usando SerpApi (solo si hay API key)
    ...
    brand_name = item.get("vendorName", "")
    if brand_name:
        return f"https://via.placeholder.com/300x300/f8f9fa/6c757d?text={brand_name.replace(' ', '+')}"
    else:
